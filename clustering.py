...
def run_xmeans(x, nmb_clusters, verbose=False):
    """Runs kmeans on 1 GPU.
    Args:
        x: data
        nmb_clusters (int): number of clusters
...
class Xmeans:
    ...
    def cluster(self, data, verbose=False):
        ...
        # replace kmeans with xmeans
        #I, loss = run_kmeans(xb, self.k, verbose)
        # may or may not need this. May be directly call xmeans
        I, loss = run_xmeans(xb, self.k, verbose)
